...
levels_income_cat <- c("low income",
                       ...
                       "upper-middle income",
                       "high income ")
waste_data_fct <- waste_data %>% mutate(
  income_cat = factor(income_cat,levels_income_cat))
...
```{r}
waste_data_long <- waste_data_fct %>% pivot_longer(cols = food_organic_waste:yard_garden_green_waste,names_to = "waste_category",values_to = "percent") %>% filter(!is.na(percent))
...
```{r}
waste_data_long_mean %>% pivot_wider(
names_from = waste_category,values_from = mean)
#Yes, this looks better, but obviously not pretty yet
```
...
```{r}
ggplot(data=waste_data_long_mean,mapping = aes(x = mean_percent,
                                               y = waste_category,
                                               fill = income_cat)) +
